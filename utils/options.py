...
class Params(object):  # NOTE: shared across all modules
    def __init__(self):
        ...
        self.save_best = (
            False
        )  # save model w/ highest reward if True, otherwise always save the latest model
        ...
        self.refs = (
            self.machine + "_" + self.timestamp
        )  # NOTE: using this as env for visdom
        ...
        self.model_file = (
            None
        )  # self.root_dir + "/models/{TODO:FILL_IN_PRETAINED_MODEL_FILE}.pth"
        if self.mode == 2:
            self.model_file = (
                self.model_name
            )  # NOTE: so only need to change self.mode to 2 to test the current training
            assert (
                self.model_file is not None
            ), "Pre-Trained model is None, Testing aborted!!!"
            self.refs = (
                self.refs + "_test"
            )  # NOTE: using this as env for visdom for testing, to avoid accidentally redraw on the training plots
        ...
        if self.visualize:
            ...
            self.logger.warning(
                "http://localhost:8097/env/" + self.refs
...
class CircuitParams(Params):  # settings for network architecture
    def __init__(self):
        ...
        if self.circuit_type == "ntm":
            ...
            self.clip_value = (
                20.0
            )  # clips controller and circuit output values to in between
    ...
    def __init__(self):
        ...
        if self.agent_type == "sl":
            if self.circuit_type == "ntm":
                ...
                self.optim_eps = (
                    1e-10
                )  # NOTE: we use this setting to be equivalent w/ the default settings in tensorflow
                self.optim_alpha = (
                    0.9
                )  # NOTE: only for rmsprop, alpha is the decay in tensorflow, whose default is 0.9
                self.eval_freq = 500
                self.eval_steps = 50
