...
class SLAgent(Agent):  # for supervised learning tasks
    ...
    def _forward(self, observation):
        ...
        for i in range(input_ts.size(0)):
            ...
            if self.mode == 2 and self.visualize:
                self.env.visual(
                    input_ts[i, 0, :].unsqueeze(0).unsqueeze(1),
        ...
        if not self.training and self.visualize:
            self.env.visual(
                input_ts, self.target_vb.data, self.mask_ts, self.output_vb.data
            )

        return (
            0
        )  # for all the supervised tasks we just return a 0 to keep the same format as rl
    ...
    def _backward(self):
        ...
        loss_vb = F.binary_cross_entropy(
            input=self.output_vb.transpose(0, 1).contiguous().view(1, -1),
    ...
    def fit_model(self):  # the most basic control loop, to ease integration of new envs
        ...
        while self.step < self.steps:
            ...
            if self.step % self.prog_freq == 0:
                ...
                self.logger.warning(
                    "Training Stats:   avg_loss:         {}".format(
                        np.mean(np.asarray(self.training_loss_avg_log))
                    )
                )
    ...
    def _eval_model(self):
        ...
        while eval_step < self.eval_steps:
            ...
            if (
                self.experience.terminal1
            ):  # or self.early_stop and (episode_steps + 1) >= self.early_stop:
                eval_should_start_new = True
        ...
        self.loss_avg_log.append([self.step, np.mean(np.asarray(eval_loss_avg_log))])
        del eval_loss_avg_log
        # plotting
        if self.visualize:
            self.win_loss_avg = self.vis.scatter(
                X=np.array(self.loss_avg_log),
                env=self.refs,
                win=self.win_loss_avg,
                opts=dict(title="loss_avg"),
            )
        # logging
        self.logger.warning(
            "Evaluation        Took: " + str(time.time() - eval_start_time)
        )
        self.logger.warning(
            "Iteration: {}; loss_avg: {}".format(self.step, self.loss_avg_log[-1][1])
        )

        # save model
        self._save_model(self.step, 0.0)  # TODO: here should pass in the negative loss
    ...
    def test_model(self):
        ...
        # Logging for this evaluation phase
        self.loss_avg_log.append([self.step, np.mean(np.asarray(test_loss_avg_log))])
        del test_loss_avg_log
        # plotting
        if self.visualize:
            self.win_loss_avg = self.vis.scatter(
                X=np.array(self.loss_avg_log),
