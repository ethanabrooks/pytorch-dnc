...
class DynamicAccessor(Accessor):
    def __init__(self, args):
        ...
        self.logger.warning(
            "<--------------------------------===> Accessor:   {WriteHead, ReadHead, Memory}"
        )
    ...
    def _reset(self):  # NOTE: should be called at __init__
        ...
        self.link_ts = torch.zeros(
            self.batch_size,
            self.write_head_params.num_heads,
            self.mem_hei,
            self.mem_hei,
        )
        self.preced_ts = torch.zeros(
            self.batch_size, self.write_head_params.num_heads, self.mem_hei
        )
        self._reset_states()
    ...
    def forward(self, hidden_vb):
        ...
        self.memory.memory_vb = self.write_heads.forward(
            hidden_vb, self.memory.memory_vb, self.usage_vb
        )
        # 3. then we update the temporal link
        self.link_vb, self.preced_vb = self.write_heads._temporal_link(
            self.link_vb, self.preced_vb
        )
        # 4. then read from memory_{t} to get read_vec_{t}
        read_vec_vb = self.read_heads.forward(
            hidden_vb,
            self.memory.memory_vb,
            self.link_vb,
            self.write_head_params.num_heads,
        )
        return read_vec_vb
