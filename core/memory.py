...
class ExternalMemory(object):
    ...
    def _reset(self):  # NOTE: should be called at each child's __init__
        self.memory_ts = torch.zeros(self.batch_size, self.mem_hei, self.mem_wid).fill_(
            1e-6
        )
        self._reset_states()
    ...
    def visual(self):
        if self.visualize:  # here we visualize the memory of batch0
            self.win_memory = self.vis.heatmap(
                self.memory_vb.data[0].clone().cpu().numpy(),
                env=self.refs,
                win=self.win_memory,
                opts=dict(title="memory"),
            )
