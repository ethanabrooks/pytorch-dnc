...
class LSTMController(Controller):
    def __init__(self, args):
        ...
        # build model
        self.in_2_hid = nn.LSTMCell(
            self.input_dim + self.read_vec_dim, self.hidden_dim, 1
        )
    ...
    def forward(self, input_vb, read_vec_vb):
        self.lstm_hidden_vb = self.in_2_hid(
            torch.cat(
                (
                    input_vb.contiguous().view(-1, self.input_dim),
                    read_vec_vb.contiguous().view(-1, self.read_vec_dim),
                ),
                1,
            ),
            self.lstm_hidden_vb,
