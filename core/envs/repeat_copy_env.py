...
class RepeatCopyEnv(Env):
    def __init__(self, args, env_ind=0):
        ...
        self.max_repeats_norm = (
            args.max_repeats_norm
        )  # to normalize the repeat flag to make it easier for the network
        self.logger.warning("Word     {length}:   {%s}", self.len_word)
        self.logger.warning(
            "Words #  {min, max}: {%s, %s}", self.min_num_words, self.max_num_words
        )
        self.logger.warning(
            "Repeats  {min, max}: {%s, %s}", self.min_repeats, self.max_repeats
        )
    ...
    def _generate_sequence(self):
        ...
        batch_repeats = np.random.randint(
            self.min_repeats, self.max_repeats + 1, size=(self.batch_size)
        )
        ...
        self.exp_state1.append(
            np.zeros(
                (
                    self.batch_size,
                    max_batch_num_words * (max_batch_repeats + 1) + 3,
                    self.len_word + 2,
                )
            )
        )  # input
        ...
        self.exp_state1.append(
            np.zeros(
                (self.batch_size, max_batch_num_words * (max_batch_repeats + 1) + 3, 1)
            )
        )  # mask
        self.unnormalize_ts = torch.ones(
            self.batch_size,
            max_batch_num_words * (max_batch_repeats + 1) + 3,
            self.len_word + 2,
        )
        for batch_ind in range(self.batch_size):
            ...
            self.exp_state1[0][batch_ind][num_words + 1][-1] = self._normalize_repeats(
                repeats
            )  # normalize the repeat flag
            self.unnormalize_ts[batch_ind][num_words + 1][
                -1
            ] = (
                self.max_repeats_norm
            )  # to ease visualization w/ unnormalized repeat flag
            # prepare target for this sample
            self.exp_state1[1][batch_ind][
                num_words + 2 : num_words * (repeats + 1) + 2, 0 : self.len_word
            ] = data_rep
            self.exp_state1[1][batch_ind][num_words * (repeats + 1) + 2][
                -1
            ] = 1  # set end bit
            # prepare mask   for this sample
            self.exp_state1[2][batch_ind][
                num_words + 2 : num_words * (repeats + 1) + 3, :
            ] = 1
