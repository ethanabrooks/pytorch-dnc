    ...
    def __init__(self, args, env_ind=0):
        ...
        if self.mode == 2:
            try:
                ...
                self.logger.warning("WARNING: scipy.misc not found")
            self.img_dir = args.root_dir + "/imgs/"
            self.frame_ind = 0
        self.seed = (
            args.seed + self.ind
        )  # NOTE: so to give a different seed to each instance
...
class GymEnv(Env):  # low dimensional observations
    ...
    def render(self):
        if self.mode == 2:
            ...
            self.logger.warning(
                "Saved  Frame    @ Step: " + str(self.frame_ind) + " To: " + frame_name
            )
            self.frame_ind += 1
            return frame
        else:
